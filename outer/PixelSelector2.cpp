...
void PixelSelector::makeHists(const FrameDatum* const fh)
{
	cout<<"ooo"<<endl;
	gradHistFrame = fh;
	//float * mapmax0 = fh->absSquaredGrad[0];

	const auto &grad = fh->imagePyramid[0].grad;

	//int w = wG[0];
	//int h = hG[0];

	int w = fh->imagePyramid[0].w;
	int h = fh->imagePyramid[0].h;
	...
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			...
			memset(hist0,0,sizeof(int)*50);// 梯度的平方根（1~49）列成的
											// 梯度的大小范围本身是0~128*128*2？
											// 不能理解
			//cout<<6<<endl;
			for(int j=0;j<32;j++) for(int i=0;i<32;i++)
			{
				...
				//int g = sqrtf(map0[i+j*w]);// 梯度的平方根
				int g = sqrtf(grad.at<float>(32 * y + j,32 * x + i));
				if(g>48) g=48;
				hist0[g+1]++;
				hist0[0]++;
			}
		...
		float* map_out, float density, int recursionsLeft, bool plot, float thFactor)
{
	cout<<"qqq"<<endl;
	float numHave=0;
	float numWant=density*fh->imagePyramid[0].w*fh->imagePyramid[0].h;
	float quotia;
	int idealPotential = currentPotential;
	cout<<1<<endl;
	cout<<numWant<<" | "<<quotia<<" | "<<idealPotential<<endl;
...
//	else
	{
		...
		numHave = n[0]+n[1]+n[2];
		quotia = numWant / numHave;
		cout<<numWant <<"--"<<numHave<<endl;

		// by default we want to over-sample by 40% just to be sure.
		float K = numHave * (currentPotential+1) * (currentPotential+1);
		idealPotential = sqrtf(K/numWant)-1;	// round down.
		if(idealPotential<1) idealPotential=1;
		cout<<recursionsLeft<<" "<<quotia<<" "<<currentPotential<<" ?"<<endl;
		//cin.get();
	...
	if(quotia < 0.95)
	{
		int wh = fh->imagePyramid[0].w*fh->imagePyramid[0].h;
		//int wh=wG[0]*hG[0];
		int rn=0;
...
Eigen::Vector3i PixelSelector::select(const FrameDatum* const fh,
		float* map_out, int pot, float thFactor)
{
	...
	auto & grad0 = fh->imagePyramid[0].grad;
	auto & grad1 = fh->imagePyramid[1].grad;
	auto & grad2 = fh->imagePyramid[2].grad;
	...
	//int h = hG[0];
	int w = fh->imagePyramid[0].w;
	int w1 = fh->imagePyramid[1].w;
	int w2 = fh->imagePyramid[2].w;
	int h = fh->imagePyramid[0].h;
		...
		for(int y3=0;y3<my3;y3+=(2*pot)) for(int x3=0;x3<mx3;x3+=(2*pot))
		{
			...
			for(int y2=0;y2<my2;y2+=pot) for(int x2=0;x2<mx2;x2+=pot)
			{
				...
				for(int y1=0;y1<my1;y1+=1) for(int x1=0;x1<mx1;x1+=1)
				{
					...
					float ag0 = grad0.at<float>(yf, xf);
					if(ag0 > pixelTH0*thFactor)
					{
						//Vec2f ag0d = map0[idx].tail<2>();
						Vec2f ag0d = Vec2f(fh->imagePyramid[0].du.at<float>(yf,xf),
											fh->imagePyramid[0].dv.at<float>(yf,xf));
					...
					float ag1 = grad1.at<float>((int)yf*0.5f+0.25f,(int)xf*0.5f+0.25f);
					...
					float ag2 = grad2.at<float>((int)(yf*0.25f + 0.125) , (int)(xf*0.25f + 0.125));
